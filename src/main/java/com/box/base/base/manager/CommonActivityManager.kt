...
class CommonActivityManager private constructor() : ActivityLifecycleCallbacks {

    companion object {

        @Suppress("StaticFieldLeak")
        private val commonActivityManager: CommonActivityManager by lazy { CommonActivityManager() }

        fun getInstance(): CommonActivityManager {
            return commonActivityManager
        }
    ...
    override fun onActivityCreated(activity: Activity, savedInstanceState: Bundle?) {
        Logs.i("%s - onCreate", activity.javaClass.simpleName)
        if (activitySet.size == 0) {
            ...
            Logs.i("%s - onApplicationCreate", activity.javaClass.simpleName)
        }
    ...
    override fun onActivityStarted(activity: Activity) {
        Logs.i("%s - onStart", activity.javaClass.simpleName)
    }

    override fun onActivityResumed(activity: Activity) {
        Logs.i("%s - onResume", activity.javaClass.simpleName)
        if (topActivity === activity && resumedActivity == null) {
            for (callback: ApplicationLifecycleCallback in lifecycleCallbacks) {
                callback.onApplicationForeground(activity)
            }
            Logs.i("%s - onApplicationForeground", activity.javaClass.simpleName)
        }
        topActivity = activity
    ...
    override fun onActivityPaused(activity: Activity) {
        Logs.i("%s - onPause", activity.javaClass.simpleName)
    }

    override fun onActivityStopped(activity: Activity) {
        Logs.i("%s - onStop", activity.javaClass.simpleName)
        ...
        if (resumedActivity == null) {
            ...
            Logs.i("%s - onApplicationBackground", activity.javaClass.simpleName)
        }
    }

    override fun onActivitySaveInstanceState(activity: Activity, outState: Bundle) {
        Logs.i("%s - onSaveInstanceState", activity.javaClass.simpleName)
    }

    override fun onActivityDestroyed(activity: Activity) {
        Logs.i("%s - onDestroy", activity.javaClass.simpleName)
        ...
        if (activitySet.size == 0) {
            ...
            Logs.i("%s - onApplicationDestroy", activity.javaClass.simpleName)
        }
    }
