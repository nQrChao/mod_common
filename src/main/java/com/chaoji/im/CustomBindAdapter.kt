...
import com.chaoji.im.ui.widget.ViewLinearLayoutCommonTitleContent
import com.chaoji.im.utils.Common
import com.chaoji.im.utils.CustomCornersTransformation
...
object CustomBindAdapter {
    ...
    fun loadRoundImage(view: ImageView, url: String) {
        if (url == "add") {
            GlideApp.with(appContext)
                .load(com.chaoji.common.R.drawable.add_white_ic)
                .transform(RoundedCorners(ResourceUtils.getDimenIdByName("idp_15")))
                .error(com.chaoji.common.R.drawable.status_error_ic)
                .into(view)
        } else {
            GlideApp.with(appContext)
                .load(url)
    ...
    fun loadRound10Image(view: ImageView, url: String?) {
        if (url.isNullOrEmpty()) {
            view.setImageResource(com.chaoji.common.R.drawable.status_error_ic)
        } else {
            val radiusInDp = 5
            val density = appContext.resources.displayMetrics.density
            val radiusInPixels = (radiusInDp * density + 0.5f).toInt()
    ...
    fun loadRound11Image(view: ImageView, drawable: Drawable?) {
        ...
        GlideApp.with(appContext)
            .load(drawable)
            .transform(CenterCrop(), RoundedCorners(radiusInPixels))
            .error(com.chaoji.common.R.drawable.image_loading_ic)
            .into(view)
    }
    ...
    fun loadImagePickMediaUri(imageView: ImageView, uri: Uri?) {
        if (uri != null) {
            ...
            imageView.setImageResource(com.chaoji.common.R.drawable.mod_goods_plus)
        }
    }
    ...
    fun loadImage(
        ...
        val glideRequest = GlideApp.with(view)
            .load(url)
            .placeholder(com.chaoji.common.R.drawable.image_loading_ic)
            .error(error ?: com.chaoji.common.R.drawable.status_error_ic)
